...
export default function AceguaMa() {
  ...
  const AcaoDoClique = (item) => {
    if (item.id == "1") {
      Speech.speak("Bagé", { language: "pt-BR" });
    } else if (item.id == "2") {
      Speech.speak("121518 habitantes ", { language: "pt-BR" });
    } else {
      Speech.speak("PIB R$3147704,99", { language: "pt-BR" });
    }
  };
